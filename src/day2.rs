...
impl PasswordPolicy {
    ...
    fn parse(string: &str) -> PasswordPolicy {
        ...
        let number_values = space_split[0]
            .split("-")
...
fn part2(input: &[PasswordPolicy]) -> i32 {
    let mut valid = 0;
    for policy in input {
        if (policy.password[(policy.range.0 - 1) as usize] == policy.character)
            ^ (policy.password[(policy.range.1 - 1) as usize] == policy.character)
        {
            valid += 1
        }
    }
...
mod tests {
    ...
    fn part1_dirty_test() {
        assert_eq!(
            part1_dirty("1-3 a: abcde\n1-3 b: cdefg\n2-9 c: ccccccccc"),
            2
        );
        assert_eq!(
            part1_dirty("1-1 a: abcde\n1-1 b: acadefg\n2-9 c: ccccccccc"),
            2
        );
    }
    #[test]
